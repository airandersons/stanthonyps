...
// /js/main.js (full version with contact form handling)
document.addEventListener('DOMContentLoaded', function () {
  // ---------- mobile menu toggle ----------
  const toggleBtn = document.getElementById('mobile-toggle');
  const mainNav = document.getElementById('mainNav');
  ...
  if (galleryItems.length > 0) {
    galleryItems.forEach((item, idx) => {
      ...
      item.addEventListener('click', () => { currentImageIndex = idx; openLightbox(idx); });
    });
  }
  ...
  if (contactForm) {
    contactForm.addEventListener('submit', function (e) {
      ...
      // simple validation (already handled by required fields)
      ...
      // Simulate sending (in a real scenario, you'd use fetch)
      showFeedback('Thank you! Your message has been sent. We’ll get back to you soon.', 'success');
      contactForm.reset(); // clear form

      // optional: hide feedback after 5 seconds
      setTimeout(() => {
